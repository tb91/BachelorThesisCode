...
my %mapNumNodes = ();
my %mapSpanningRatioGG = ();
my %mapSpanningRatioPDT = ();
...
sub readFileAndSaveValsInHashMaps {
	...
	while (<FILE>) 
	{
		...
		my ($density, $numNodes, $ratioGG, $ratioPDT, $ratioExt, $ratioExtShortest, $sentNN, $sentReq, $sentRep, $sentVirt, $sentNNExt, $sentReqExt, $sentRepExt, $sentVirtExt, $sentNWExt, $isSymetric, $isSymetricExt, $seeed) = split($IN_SPLIT_CHAR);
		...
		push(@{$mapSpanningRatioGG{$density}}, $ratioGG);
		push(@{$mapSpanningRatioPDT{$density}}, $ratioPDT);
...
foreach my $density ( sort {$a <=> $b} keys %mapNumNodes ) #presumption: all maps have the same keys
{
	...
	print OUT "\n" or die "$error_write_msg";  
	estimateConfidenceIntervals("GG_spanningratio", $density, @{$mapSpanningRatioGG{$density}});
	print OUT "\n" or die "$error_write_msg";  
	estimateConfidenceIntervals("PDT_spanningratio", $density, @{$mapSpanningRatioPDT{$density}});
	print OUT "\n" or die "$error_write_msg";
...
foreach my $density ( sort {$a <=> $b} keys %mapNumNodes ) #presumption: all maps have the same keys
{
	...
	estimateConfidenceIntervals("", $density, @{$mapSpanningRatioGG{$density}});
	print OUT "$OUT_SPLIT_CHAR" or die "$error_write_msg";
	
	estimateConfidenceIntervals("", $density, @{$mapSpanningRatioPDT{$density}});
	print OUT "$OUT_SPLIT_CHAR" or die "$error_write_msg";
}
close(OUT);
